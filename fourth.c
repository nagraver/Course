...
void print_roster(roster **begin) {
    ...
    for (roster *current = *begin; current; current = current->next) {
        printf("| %16s | %16.2f | %16.2f |\n", current->info.name, current->info.cost, current->info.rating);
    }
}
...
void load_roster(roster **begin) {
    ...
    data content;
    while (*begin) clear_first(begin);
    while (fread(&content, sizeof(data), 1, file)) add_last(begin, content);
    ...
    for (roster *current = *begin; current; current = current->next) {
        for (roster *buffer = current->next; buffer; buffer = buffer->next) {
            if (current->info.cost == buffer->info.cost) {
                ...
                printf("| %14s | %14.2f | %14.2f |\n", buffer->info.name, buffer->info.cost, buffer->info.rating);
                flag = 1;
            }
        }
    }

    if (flag == 0) puts("Повторяющихся элементов нет");
}
